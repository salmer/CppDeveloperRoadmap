...
def main():
    ...
    dsl.append("# (map coordinate units). `arrow=left|right|top|bottom` (required) sets which box edge the")
    dsl.append("# arrow starts from. Hand-tuned; keep `mapcheck` clean across all languages after edits.")
    for hid in hint_ids:
        ...
        # (draw.io is the output) instead of re-deriving it. 0deg = right, 90deg = up.
        # arrow = which box edge the arrow starts from (the edge facing the target).
        ...
        dx, dy = tcx - hcx, tcy - hcy
        arrow = ("right" if dx >= 0 else "left") if abs(dx) >= abs(dy) else ("bottom" if dy >= 0 else "top")
        dsl.append(f"hint [{key[hid]}] angle={ang} dist={dist} arrow={arrow} -> " + ", ".join(targets))
